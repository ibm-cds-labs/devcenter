...
var newDocument = function(callback) {
  load(function(err, schema) {
    var doc = require('./fixedfields.js')
    if (!err) {
      for (var i in schema) {
...
var generateSearchIndex = function(schema, callback) {
  ...
  couchmigrate.migrate("devcenter", ddoc, callback);
};
...
load(function(err, data) {
  if (err) {
    var schema = {};
    save(schema, function(err, data) {
      generateSearchIndex(schema, function() {
